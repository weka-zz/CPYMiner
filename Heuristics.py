...
if __name__ == "__main__":
    ...
    #funcs = json.load(open(r"E:\LLM\parameters.json", 'r'))
    funcs = json.load(open(r"D:\all_ida_json\gcc-7.5.0-O0-mips.json", 'r'))
    memcpy=['alps_lib_toupper', 'blt_str_utf8_cpy', 'sstrncpy', 'strlcpy', 'memcpy', 'memmove', 'strcpy', 'strcat', 'strncpy', 'strncat', 'memccpy', 'wcsncat', 'wcsxfrm', 'wcscat', 'wmemcpy', 'wcscpy', 'wmemmove', 'wcsncpy', 'mempcpy']

    func_names = [f for f in funcs]
    print(len(func_names))
    # for fun in idc.function():
    callees=0
    ...
    # for i in all_func_param:
        for func_ea in idautils.Functions():
            ...
            call_count=count_function_calls(func_ea)
            #print(call_count)
            # if ((contains_loop(func_ea) and call_count <= 4 or call_count>0 and call_count <= 4 and contains_loop(func_ea)==False)):
                # if  i in func_names :
                    # j=0
                    # for a, b in enumerate(all_func_param[i]):
                    ...
                    #             j=j+1
                    # if j>1 and len(all_func_param[i])<=4:
            if contains_loop(func_ea) and call_count <= callees:#还需要分析下，直接内存拷贝函数是通过循环结构体实现
                Potential_mem_funlist.append(func_name)
            if call_count>0 and call_count <= callees and contains_loop(func_ea)==False:
                Potential_mem_funlist.append(func_name)
