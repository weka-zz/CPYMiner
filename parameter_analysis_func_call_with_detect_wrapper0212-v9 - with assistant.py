...
def generate_copy_function_prompt(code,params,calles,copy_fun):
    copy_fun_info=[]
    assemble={}
    subfun=[]
    if calles:
    ...
    if calles:
        ...
        4. Only provide the final result:
        ...
        1.The function's parameters and types:{params}.
        2.The function's assembly code:\n{code}. 
        3. Only provide the final result:
           - If it is a copy function, answer: "Yes".
           - If it is not, answer: "No".
        """
...
def analyze_assembly_code_B(assembly_code,parameters,calles,copy_fun):
    ...
    try:
        ...
        response = clientB.chat.completions.create(
            model="gpt-4o",  # 或使用 "gpt-3.5-turbo" 以降低成本
            messages=[
                {"role": "system",
            ...
            temperature=0.1,  # 温度值低可以生成更准确的回答
            top_p =1
        )
...
def analyze_functions(client, assistant_id, thread_id, full_code,params, callees, copy_fun):
    """
    依次分析多个汇编函数，GPT-4 依据 `thread_id` 记住已分析的函数，提高效率。
    """
    prompt = generate_copy_function_prompt(full_code, params, callees, copy_fun)
    #print(prompt)
...
if __name__ == "__main__":
    ...
    memcpy=['sub_4004B0', 'sub_401C98', 'sub_402710', 'sub_403118', 'sub_4282D0', 'sub_428410', 'sub_4284A0', 'sub_4285A0', 'sub_428730', 'sub_428790', 'sub_428890', 'sub_428940', 'sub_428990', 'sub_4289E0', 'sub_428A20', 'sub_428A50', 'sub_428A80', 'sub_428AE0', 'sub_430E30']

    func_names = [f for f in funcs]
    # for callees in range(0, 5, 1):
    callees=1
    Potential_mem_funlist = []
    for i in all_func_param:
        func_ea = idc.get_name_ea_simple(i)
        call_count=count_function_calls(func_ea)
        if ((contains_loop(func_ea) and call_count <= callees or call_count>0 and call_count <= callees and contains_loop(func_ea)==False)):
            # if  i in func_names :
            j=0
            for a, b in enumerate(all_func_param[i]):
    ...
    for fun in memcpy:
        if fun not in Potential_mem_funlist:
            print(fun)

    os.environ['http_proxy'] = 'socks5://127.0.0.1:10443'
    os.environ['https_proxy'] = 'socks5://127.0.0.1:10443'
    openai.api_key = 'xxx'
    ...
    #     prompt = """
    #         You will be provided with the assembly code of a function. Your task is to analyze the assembly code and determine whether the function qualifies as a copy function. Follow the criteria below and provide only "Yes" or "No" as the final answer.
    #         ---
    #
    ...
    #         - **Important**: If the function performs encoding conversion (e.g., through table lookup or bit manipulation) during the copying process, it should still be considered a copy function.
    #
    ...
    #         - **Parameter Passing**: Parameters may be passed between registers (e.g., '$a0', '$a1', '$a2', '$v0', '$v1') and memory.
    ...
    #         ---
    #
    #         ### **6. Exclude the following function types from classification as copy functions:**
    #            - **Memory Initialization**: Functions that only initialize memory or write fixed values (e.g., `memset`, setting memory to `0xFFFFFFFF`).
    #            - **Encryption or Encoding**: Functions that perform encryption, encoding, or cryptographic transformations (e.g., `xor`, `sha`, `md5`).
    #            - **Mathematical Operations**: Functions that compute values unrelated to data copying (e.g., checksums, hashing).
    #            - **Comparison Functions**: Functions that compare data but do not transfer it (e.g., `strcmp`, `memcmp`).
    #
    #         ---
    #
    #         ### **7. Handling Subfunctions**
    #            ✅ **Calling a known copy function qualifies the function as a copy function.**
    #            - If the function calls a **recognized copy function** (`memcpy`, `strcpy`, `wcscpy`, `__strcpy_chk`, `__memcpy_chk`, `__strcat_chk`, `__wcscpy_chk`, `__wcsncat_chk`), it inherits the copy function classification.
    #            - **If the primary function lacks explicit load/store instructions but its subfunction performs data transfer, it is still a copy function.**
    #
    #            ✅ **Calling `__chk_fail` does NOT disqualify a function as a copy function.**
    #            - Secure copy functions include safety checks (e.g., `__chk_fail`) for buffer overflow protection.
    #            - **Important**: If `__chk_fail` is invoked only upon detecting an overflow, it does not affect whether the function qualifies as a copy function.
    #            - These functions behave similarly to their standard counterparts (e.g., `strcpy`, `memcpy`), with additional safety mechanisms.
    #
    #            🚨 **A function is NOT a copy function if it only calls:**
    #            - **Comparison functions** (`strcmp`, `memcmp`) that only check data.
    #            - **Index management functions** (e.g., `log_index_update`) that adjust buffer indices.
    #            - **Memory clearing functions** (`memset`) when used alone without a subsequent copy operation.
    #            - **Unknown functions** with an undetermined purpose.
    #
    #         ---
    #
    #         ### **Function Evaluation**
    #         1.The function's parameters and types:{params}.
    #         2. **Function Assembly Code:** {code}.
    #         3. **Final Decision:**
    #            - If the function qualifies as a copy function, respond with: `"Yes"`.
    #            - If it does **not** qualify, respond with: `"No"`.
    #            - **Provide no explanations—only return `"Yes"` or `"No"` as the final answer.**
    # """

        print("🔹 创建 AI Assistant ...")
        assistant = clientA.beta.assistants.create(
            name="Assembly Copy Function Analyzer",
            instructions=prompt,
            model="gpt-4o",
            temperature=0,
            top_p=1
        ...
        print("🔹 创建对话线程 ...")
        thread = clientA.beta.threads.create()
        ...
        for funcname in Potential_mem_funlist:
            print(f"🔍 正在分析函数：{funcname} ...")
            assembly = gen_disasm(funcname)
            assembly_code = "\n".join(assembly)
            assembly_code = funcname + "\n" + assembly_code
            calles=get_callee(funcname)

            copy_fun = analyze_functions(clientA, assistant_id, thread_id, assembly_code, all_func_param[funcname], calles,copy_fun)
